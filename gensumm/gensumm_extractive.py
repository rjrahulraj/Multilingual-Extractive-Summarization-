...
# gensumm_extractive.py
"""
Sentence splitting + Sentence Embeddings (GPU-Optimized)

This module provides:
- split_sentences(text) → list[str]
- embed_sentences(sentences) → torch.Tensor (GPU)
"""
...
# ------------------------------------------------------------------
# 1. Load SBERT model ONCE (on GPU)
# ------------------------------------------------------------------

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
print(f"[Embedding] Using device: {DEVICE}")

# You may replace this with LaBSE or any multilingual model
MODEL_NAME = "sentence-transformers/all-MiniLM-L6-v2"

# Load SBERT on GPU
embedder = SentenceTransformer(MODEL_NAME, device=DEVICE)
# Works for all versions of sentence-transformers
try:
    sample_param = next(embedder[0].parameters())
    print("[Embedder] Loaded on device:", sample_param.device)
except Exception:
    try:
        print("[Embedder] Model device:", embedder.device)
    except:
        print("[Embedder] Device check skipped.")
...
# ------------------------------------------------------------------
# 3. Sentence Embedding (GPU-Optimized)
# ------------------------------------------------------------------

def embed_sentences(sentences: List[str]):
    """
    Returns GPU tensor embeddings for input sentences.

    Output shape → (num_sentences, 384)
    """
